...
public class Heap {
    ...
    public void insert(int val){
        ...
        while (index > 1){
            ...
            System.out.println("Successfully inserted " + arr[index]);
        }
    }
    public void deleteFromHeap(){
        if(size == 0){
            System.out.println("\nNothing to delete. Empty Heap.");
            return;
        }
    ...
    public static void heapSort(int[] arr, int n){
        ...
        while(size > 1){
            ...
            arr[size] = temp;
            size--;
//the root value at the index 1 might not be at it's correct position
//call the heapify method to correct the position 1
    ...
    public static void main(String[] args) {
        ...
        int n = 5;
        for(int i = n/2; i > 0; i--){ //only non-leaf nodes are the target here
            Heapify(arr, n, i);
        }
        System.out.println("\nArray after heapifying: ");
        for(int i =1; i<=n; i++){
            System.out.print(arr[i] + " ");
        }
        System.out.println();

        System.out.println("Array elements after the Heap Sort: (n log n): ");
        heapSort(arr, n);
        for( int i =1; i<=n; i++){
            System.out.println();
        }
